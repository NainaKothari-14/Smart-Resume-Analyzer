...
function saveHistory(result, role) {
...
function Pill({ label, type }) {
  const styles = {
    ...
    jd:      { background:"rgba(245,166,35,0.12)", color:"#f5a623", border:"1px solid rgba(245,166,35,0.3)" },
  };
...
function SectionBadge({ label, ok }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 13px", borderRadius:8, fontSize:12, fontWeight:500,
      background: ok?"rgba(0,200,150,0.08)":"rgba(255,80,80,0.07)",
      border:`1px solid ${ok?"rgba(0,200,150,0.3)":"rgba(255,80,80,0.2)"}`, color:ok?"#00c896":"#5a5f6e" }}>
...
function Card({ title, children, style={}, accent }) {
  return (
    <div style={{ background:"linear-gradient(145deg, #17191e, #13151a)", border:`1px solid ${accent||"#252830"}`, borderRadius:14, padding:"1.3rem", marginBottom:14, ...style }}>
      {title && <div style={{ fontSize:10, textTransform:"uppercase", letterSpacing:"0.1em", color:accent||"#4a4f60", marginBottom:14, fontWeight:600 }}>{title}</div>}
      {children}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(() => { if (result) rightRef.current?.scrollIntoView({ behavior:"smooth", block:"start" }); }, [result]);

  async function handleAnalyze() {
    if (!resumeText.trim()) { setError("Please paste your resume text first."); return; }
    setLoading(true); setError(""); setResult(null);
    try {
      const data = await analyzeText(resumeText, role, jdText);
      setResult(data);
      setHistory(saveHistory(data, role));
  ...
  async function handleFile(file) {
    if (!file) return;
    setLoading(true); setError(""); setResult(null);
    try { const data = await uploadFile(file, role); setResult(data); setHistory(saveHistory(data, role)); }
    catch(e) { setError(e.message); }
    finally { setLoading(false); }
  }
  ...
  async function downloadPDF() {
    if (!result) return;
    setDownloading(true);
    try { await generatePDF(result); }
    catch(e) { alert("PDF generation failed: "+e.message); }
    finally { setDownloading(false); }
  }

  const scoreColor = (v) => v >= 70 ? "#00c896" : v >= 45 ? "#f5a623" : "#ff7070";
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#0b0c0f", color:"#e8eaf0", fontFamily:"'DM Sans', sans-serif" }}>
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:result?"380px 1fr":"1fr", maxWidth:result?1060:560, margin:"0 auto", padding:"0 1.5rem 5rem", gap:18, alignItems:"start" }}>
          ...
          <div style={{ position:"sticky", top:20 }}>
            ...
            {/* Score History */}
            {history.length > 0 && (
              <Card title="Score History" style={{ marginBottom:14 }}>
                {history.map((h, i) => (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 0", borderBottom: i < history.length-1 ? "1px solid #1a1c22":"none" }}>
                    <div>
                      <div style={{ fontSize:12, color:"#e8eaf0", fontWeight:500 }}>{h.date} · {h.time}</div>
                      <div style={{ fontSize:11, color:"#4a4f60", marginTop:2 }}>{h.role}</div>
                    </div>
                    <div style={{ display:"flex", gap:8 }}>
                      <span style={{ fontSize:13, fontWeight:700, color:scoreColor(h.atsScore) }}>{h.atsScore}%</span>
                      {i > 0 && (
                        <span style={{ fontSize:11, color: h.atsScore > history[i-1]?.atsScore ? "#00c896":"#ff7070" }}>
                          {h.atsScore > history[i-1]?.atsScore ? "▲":"▼"}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {roles.map(r => (
                  <button key={r.key} onClick={() => setRole(r.key)} style={{
                    ...
                    color:role===r.key?"#fff":"#5a5f6e", fontWeight:role===r.key?600:400,
            ...
            <Card>
              ...
              <textarea value={resumeText} onChange={e=>setText(e.target.value)}
                ...
                style={{ width:"100%", minHeight:150, background:"#0e0f12", border:"1px solid #252830", borderRadius:8, padding:13, fontSize:12.5, fontFamily:"'DM Mono',monospace", color:"#c8cad4", resize:"vertical", marginBottom:10, boxSizing:"border-box", outline:"none", lineHeight:1.7 }}/>

              {/* JD Toggle */}
              <button onClick={() => setShowJD(!showJD)} style={{ background:"transparent", border:"none", color:"#6c63ff", fontSize:12, fontWeight:600, cursor:"pointer", padding:"4px 0", marginBottom:10, fontFamily:"'DM Sans',sans-serif" }}>
                {showJD ? "▲ Hide" : "▼ Add"} Job Description (optional)
              </button>

              {showJD && (
                <textarea value={jdText} onChange={e=>setJdText(e.target.value)}
                  placeholder={"Paste the job description here...\n\nThe system will match your resume keywords against JD requirements."}
                  style={{ width:"100%", minHeight:120, background:"#0e0f12", border:"1px solid #6c63ff44", borderRadius:8, padding:13, fontSize:12.5, fontFamily:"'DM Mono',monospace", color:"#c8cad4", resize:"vertical", marginBottom:10, boxSizing:"border-box", outline:"none", lineHeight:1.7 }}/>
              )}

              <button onClick={handleAnalyze} disabled={loading} style={{
                width:"100%", padding:14, background:loading?"#3a3860":"linear-gradient(135deg, #6c63ff, #5048d4)",
          ...
          {result && (
            <div ref={rightRef} style={{ animation:"fadeUp 0.45s ease both" }}>
              ...
              {/* Strength Line + Best Role */}
              <Card accent="#6c63ff44">
                <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
                  <span style={{ fontSize:22 }}>💡</span>
                  <div>
                    <div style={{ fontSize:13, color:"#c8cad4", lineHeight:1.6, marginBottom:10 }}>{result.strengthLine}</div>
                    {result.bestRole && (
                      <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
                        <span style={{ fontSize:11, color:"#4a4f60", textTransform:"uppercase", letterSpacing:"0.07em" }}>Best suited role:</span>
                        <span style={{ padding:"3px 12px", borderRadius:999, fontSize:12, fontWeight:700, background:"rgba(108,99,255,0.15)", color:"#9d97ff", border:"1px solid rgba(108,99,255,0.3)" }}>
                          {result.bestRole.label}
                        </span>
                        <span style={{ fontSize:11, color:"#4a4f60" }}>{result.bestRole.score}% match</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* JD Match */}
              {result.jdMatch && (
                <Card title={`JD Match — ${result.jdMatch.jdScore}% keyword match`} accent="#f5a62344">
                  <div style={{ marginBottom:12 }}>
                    <div style={{ fontSize:11, color:"#4a4f60", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.07em" }}>Keywords matched from JD</div>
                    <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                      {result.jdMatch.matched.map(s => <Pill key={s} label={s} type="found"/>)}
                    </div>
                  </div>
                  {result.jdMatch.missing.length > 0 && (
                    <div>
                      <div style={{ fontSize:11, color:"#4a4f60", marginBottom:8, textTransform:"uppercase", letterSpacing:"0.07em" }}>JD keywords missing from resume</div>
                      <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
                        {result.jdMatch.missing.map(s => <Pill key={s} label={s} type="jd"/>)}
                      </div>
                    </div>
                  )}
                </Card>
