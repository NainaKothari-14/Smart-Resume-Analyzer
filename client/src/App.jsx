...
function Ring({ value, size = 130, stroke = 10, label, color, glow }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
      <div style={{ position: "relative", width: size, height: size }}>
        ...
        {/* Text centered absolutely inside ring */}
        <div style={{
...
function Pill({ label, type }) {
  ...
  return (
    <span style={{ padding: "4px 13px", borderRadius: 999, fontSize: 12, fontWeight: 500, ...styles[type] }}>
      {label}
    </span>
  );
}
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        background: ok ? "#00c896" : "#3a3d45",
        boxShadow: ok ? "0 0 6px #00c89688" : "none",
      }} />
      {label}
    </div>
  );
...
function Card({ title, children, style = {} }) {
  return (
    <div style={{
      background: "linear-gradient(145deg, #17191e, #13151a)",
      border: "1px solid #252830",
      borderRadius: 14, padding: "1.3rem",
      marginBottom: 14, ...style,
    }}>
      {title && (
        <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: "#4a4f60", marginBottom: 14, fontWeight: 600 }}>
          {title}
        </div>
      )}
      {children}
          ...
          <div style={{ position: "sticky", top: 20 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {roles.map(r => (
                  <button key={r.key} onClick={() => setRole(r.key)} style={{
                    padding: "6px 15px", borderRadius: 999, fontSize: 12,
            ...
            <Card>
              ...
              <div
                ...
                style={{
                  border: `1.5px dashed ${drag ? "#6c63ff" : "#252830"}`,
                  borderRadius: 10, padding: "1.6rem", textAlign: "center", cursor: "pointer",
                  background: drag ? "rgba(108,99,255,0.07)" : "rgba(255,255,255,0.01)",
                  marginBottom: 14, transition: "all 0.2s",
                }}
              >
                <div style={{ fontSize: 30, marginBottom: 8, filter: "grayscale(0.3)" }}>📄</div>
              ...
              <textarea
                ...
                style={{
                  width: "100%", minHeight: 165, background: "#0e0f12",
                  border: "1px solid #252830", borderRadius: 8, padding: 13,
                  fontSize: 12.5, fontFamily: "'DM Mono', monospace",
                  color: "#c8cad4", resize: "vertical", marginBottom: 14,
              ...
              {error && (
                <div style={{ marginTop: 12, background: "rgba(255,80,80,0.08)", border: "1px solid rgba(255,80,80,0.25)", borderRadius: 8, padding: "11px 14px", fontSize: 13, color: "#ff7070" }}>
                  ⚠ {error}
                </div>
              )}
            </Card>
            ...
            <div ref={rightRef} style={{ animation: "fadeUp 0.45s ease both" }}>
              ...
              <Card title="Score Overview">
                <div style={{ display: "flex", justifyContent: "space-around", flexWrap: "wrap", gap: 20, padding: "0.5rem 0" }}>
                  {RINGS.map(({ key, label, color }) => (
                    <Ring key={key} value={result[key]} label={label} color={color} />
                  ))}
                </div>
              </Card>
                ...
                <Card title="Skill Categories">
                  {Object.entries(result.skills.byCategory).map(([key, cat], i, arr) => (
                    <div key={key} style={{
                      ...
                      padding: "9px 0", borderBottom: i < arr.length - 1 ? "1px solid #1a1c22" : "none",
                    }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ width: 90, height: 4, background: "#1e2026", borderRadius: 999, overflow: "hidden" }}>
                          <div style={{
                            ...
                            width: `${Math.min(100, cat.count * 13)}%`,
              ...
              <Card title={`Role Analysis — ${result.roleAnalysis.role}`}>
                {result.roleAnalysis.requiredFound.length > 0 && (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                      {result.roleAnalysis.requiredFound.map(s => <Pill key={s} label={s} type="found" />)}
                    </div>
                  </div>
                )}
                {result.roleAnalysis.requiredMissing.length > 0 && (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                      {result.roleAnalysis.requiredMissing.map(s => <Pill key={s} label={s} type="missing" />)}
                    </div>
                  </div>
                )}
                {result.roleAnalysis.preferredMissing.length > 0 && (
                  <div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                      {result.roleAnalysis.preferredMissing.map(s => <Pill key={s} label={s} type="cat" />)}
                    </div>
                  </div>
                )}
              </Card>
              ...
              <Card title="Resume Sections">
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                  {Object.entries(result.sections).map(([key, ok]) => (
                    <SectionBadge key={key} label={sectionLabels[key]} ok={ok} />
                  ))}
                </div>
              </Card>
                ...
                <Card title="Suggestions to Improve">
                  {result.suggestions.map((s, i) => (
                    <div key={i} style={{
                      ...
                      borderBottom: i < result.suggestions.length - 1 ? "1px solid #1a1c22" : "none",
                      fontSize: 13, color: "#7a7f8e", lineHeight: 1.6,
                    }}>
                      <span style={{
                        width: 22, height: 22, borderRadius: 6, flexShrink: 0,
                        background: "rgba(245,166,35,0.12)", border: "1px solid rgba(245,166,35,0.25)",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 11, color: "#f5a623", fontWeight: 700, marginTop: 1,
                      }}>{i + 1}</span>
                      {s}
                    </div>
                  ))}
              ...
              <div style={{ textAlign: "center", fontSize: 11, color: "#2a2d35", marginTop: 6 }}>
